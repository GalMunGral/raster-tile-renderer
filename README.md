...
Let $`(x, y, 2^z)`$ be the representative of $`\mathbf{f}`$ at zoom $`z`$. The tile containing $`\mathbf{f}`$ satisfies $`2^{z-Z} N X \leq x < 2^{z-Z} N(X+1)`$, giving $`X = \lfloor x / (2^{z-Z} N) \rfloor`$. Its left edge is at canvas $`c_x - (x \bmod 2^{z-Z} N)`$. From the placement formula, tile $`(X + \Delta X, Y + \Delta Y, Z)`$ is offset by $`2^{z-Z}(N \Delta X,\ N \Delta Y)`$ from tile $`(X, Y, Z)`$ on screen. The visible range is all $`(\Delta X, \Delta Y)`$ for which that offset still intersects the canvas.